...
#fil = [{'Name': 'owner-id', 'Values': ['274138180186']}]
    ...
    #np_sers_ids.append
np_sers_ids = []   
for each_item in ec2_con_cli.describe_dhcp_options()['DhcpOptions']:
         print(each_item['DhcpOptionsId'])
...
for key in s3_con_cli.list_objects(Bucket=bucketname)['Contents']:
     ...
     owner-id-filter = [{'Name': 'owner-id', 'Values': ['274138180186']}]
     for each_item in ec2_con_cli.describe_snapshots(Filters = fil)['Snapshots']:
         print(each_item['SnapshotId'])

     # Logic for Get list of EC2 AMI IDs
     owner-id-filter = [{'Name': 'owner-id', 'Values': ['274138180186']}]
     for each_item in ec2_con_cli.describe_images(Filters = owner-id-filter)['Images']:
     ...
     for each_item in ec2_con_cli.describe_volumes()['Volumes']:
         np_sers_ids.append(each_item['VolumeId'])


     # Logic for Get list of EC2 describe_vpc_peering_connections VpcPeeringConnectionId
     for each_item in ec2_con_cli.describe_vpc_peering_connections()['VpcPeeringConnections']:
         np_sers_ids.append(each_item['VpcPeeringConnectionId'])

     # Logic for Get list of EC2 describe_subnets SubnetId
     for each_item in ec2_con_cli.describe_subnets()['Subnets']:
         np_sers_ids.append(each_item['SubnetId'])

     # Logic for Get list of EC2 describe_route_tables RouteTableId
     for each_item in ec2_con_cli.describe_route_tables()['RouteTables']:
         np_sers_ids.append(each_item['RouteTableId'])
         
     # Logic for Get list of EC2 describe_dhcp_options DhcpOptions
     for each_item in ec2_con_cli.describe_dhcp_options()['DhcpOptions']:
         np_sers_ids.append(each_item['DhcpOptionsId'])

     #ec2_con_res.create_tags(Resources= np_sers_ids, Tags= Tag_var, )
  
#this is for get Account ID
